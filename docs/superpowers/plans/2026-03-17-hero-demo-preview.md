# Hero Demo Preview Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a looping animated product-tour preview to the landing page hero and animate the "What it does" feature rows on scroll.

**Architecture:** Two new client component islands (`HeroDemoPreview`, `FeatureRow`) imported dynamically from `page.tsx`. `HeroDemoPreview` runs a 14s scene loop (feed → tailor → pipeline) using framer-motion `AnimatePresence`. `FeatureRow` uses `useInView` to trigger scroll-entrance animations. `SignedOutHero` is restructured into a two-column grid.

**Tech Stack:** framer-motion, Next.js 15 `dynamic()`, React `useEffect`/`useState`/`useRef`
...
|------|--------|----------------|
| `src/components/landing/HeroDemoPreview.tsx` | Create | Looping 3-scene demo: feed, tailor, pipeline |
| `src/components/landing/FeatureRow.tsx` | Create | Animated feature row with scroll-triggered entrance |
| `src/app/page.tsx` | Modify | Two-column hero layout, dynamic imports, mobile fixes |

---

## Chunk 1: Dependency + FeatureRow
...
### Task 2: Create `FeatureRow` client component

**Files:**
- Create: `src/components/landing/FeatureRow.tsx`
- Modify: `src/app/page.tsx` (remove inline `FeatureRow`, import new one)
...
- [ ] **In `src/app/page.tsx`, delete the existing `FeatureRow` function** (lines ~603–627 — the entire `function FeatureRow(...)` block).

- [ ] **Add the import at the top of `page.tsx`** (with existing imports):
...
Expected: no errors. The `FEATURES` array already passes the right props — no changes needed there.

- [ ] **Commit**

```bash
git add src/components/landing/FeatureRow.tsx src/app/page.tsx
git commit -m "Add animated FeatureRow with scroll-triggered entrance"
```

---

## Chunk 2: HeroDemoPreview component

### Task 3: Create `HeroDemoPreview`

**Files:**
- Create: `src/components/landing/HeroDemoPreview.tsx`

- [ ] **Create `src/components/landing/HeroDemoPreview.tsx`** with the full content below:
...
```bash
git add src/components/landing/HeroDemoPreview.tsx
git commit -m "Add HeroDemoPreview animated demo loop"
```

---

## Chunk 3: page.tsx integration

### Task 4: Restructure `SignedOutHero` and wire everything up

**Files:**
- Modify: `src/app/page.tsx`

This task makes four changes to `page.tsx`:
1. Add `dynamic` imports for `HeroDemoPreview`
2. Extract the stats row markup into a `StatsRow` function so it can be rendered twice (mobile/desktop placement differs)
3. Restructure `SignedOutHero` into a two-column grid
4. Fix mobile stats row: always horizontal, never stacked

- [ ] **Add `dynamic` import** — add these lines near the top of `page.tsx`, after the existing `import` statements:
...
- [ ] **Extract `StatsRow` as a helper function** — add this new function directly above `SignedOutHero` in `page.tsx`:
...
- [ ] **Replace the `SignedOutHero` function** entirely with the two-column version below. Find and delete the existing `function SignedOutHero()` and replace with:
...
- [ ] **Confirm no remnant stats row** — the replacement of `SignedOutHero` in the previous step already eliminates the old stats row (it was inside the function that was replaced). Do a quick search to confirm nothing remains:

```bash
grep -n "flex-col gap-5 sm:flex-row" src/app/page.tsx
```
...
- [ ] **Verify in browser** — run dev server and check:
  - Desktop (≥1024px): two-column hero, preview on right, stats below CTAs on left
  - Mobile (<1024px): single column, preview appears between CTAs and stats
  - Stats row is always horizontal (never stacked)
  - Feature rows animate in as you scroll down
  - Demo preview loops: feed (5s) → tailor (5s) → pipeline (4s) → repeat
...
git commit -m "Restructure hero into two-column layout with animated demo preview"
```
